...
def create_favorite(
    ...
    exist_fav = db.query(Favorite).filter(Favorite.user_id == favorite_data.user_id, Favorite.title == favorite_data.title).first()

    if not user:
        raise HTTPException(status_code=404, detail="Usuário não encontrado.")
    if exist_fav:
        raise HTTPException(status_code=400, detail="Favorito já adicionado.")
...
def delete_favorite(favorite_id: int, db: Session = Depends(get_session)):
    favorite = db.query(Favorite).filter(Favorite.id == favorite_id).first()
    ...
    return {"message": f"Favorito {favorite_id} deletado."}